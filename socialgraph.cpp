...
vector<string> SocialGraph::Suggestions(string user){
    ...
    int user_loc = search(users,user);
    vector<string> res;
    for(auto id : followers[user_loc]){

        int follower_loc =search(users,id);

        for(auto F_of_F_id : followers[follower_loc]){
            string name = users[search(users,F_of_F_id)].name;
    ...
    for(auto name:res){
        ...
        for(auto id :getFollowings()[user_loc]){
...
vector<vector<string>> SocialGraph::StringList(){
    ...
    for(auto followers_list : getFollowings()/*followers*/){
