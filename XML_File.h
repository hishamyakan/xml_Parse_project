 ...
 *                            Global Variables                                 *
 *******************************************************************************/
static string aligned_str = "";
static string minified_str = "";
static string json_str = "";
static bool prevArrayNotation = false;
...
public:
            ...
            for(int i = 0 ; i < (int)words.size() ; i++) {
                ...
                /*If it's an closing one: */
                    ...
                    if(tag_stack.isEmpty() || !areMatching( tag_stack.top(), word) ){
                        cout<<"Error location is on line "<<error_loc<<endl ;
                        return false;
                    }
    ...
    /*
        ...
        SocialGraph toGraph(){
                    file_struct = correctTree(file_path);
                    vector<User> u =  getUsers(file_struct.root);
                    SocialGraph graph(u);
                    return graph;
                }
